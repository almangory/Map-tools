              ...
              >
                  ...
                  >
                      ...
                      <div className="p-6 sm:p-8 overflow-y-auto space-y-6 flex-1 custom-scrollbar text-white">
                                 ...
                                 <div className="flex items-center gap-3 text-accent min-w-0">
                                    ...
                                    <div className="min-w-0">
                                       ...
                                       <p className="text-[10px] text-white/70 mt-0.5">
                                          {lang === 'ar' ? 'تثبيت التطبيق على الشاشة الرئيسية للوصول السريع بدون متصفح' : 'Install the app on home screen for quick offline-ready access'}
                                       </p>
                                    </div>
                                 </div>
                                 <button
                          ...
                          {/* 1. Language & Display */}
                          <div className="space-y-3">
                              <h4 className="text-xs font-bold text-accent uppercase tracking-wider">{lang === 'ar' ? 'اللغة والمظهر العام' : 'Language & Display'}</h4>
                              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                  <div className="bg-black/30 p-3.5 rounded-xl border border-white/5 flex items-center justify-between">
                                      <span className="text-xs font-semibold">{lang === 'ar' ? 'لغة الواجهة' : 'Interface Language'}</span>
                                      <div className="flex bg-white/5 p-1 rounded-lg border border-white/10">
                                          <button onClick={() => setLang('ar')} className={cn("px-2.5 py-1 text-xs rounded font-bold transition-all", lang === 'ar' ? "bg-accent text-[#071720]" : "text-white/60 hover:text-white")}>عربي</button>
                                  ...
                                  <div className="bg-black/30 p-3.5 rounded-xl border border-white/5 flex items-center justify-between">
                                      <span className="text-xs font-semibold">{lang === 'ar' ? 'نمط الخريطة الافتراضي' : 'Default Map Type'}</span>
                                      <select
                                          value={globalBaseMap}
                          ...
                          <div className="space-y-3">
                              <h4 className="text-xs font-bold text-accent uppercase tracking-wider">{lang === 'ar' ? 'الإعدادات الهندسية ونظام الإحداثيات' : 'Geodetic & Engineering Settings'}</h4>
                              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                  <div className="bg-black/30 p-3.5 rounded-xl border border-white/5 flex items-center justify-between">
                                      <span className="text-xs font-semibold">{lang === 'ar' ? 'نظام الإحداثيات الافتراضي' : 'Default Projection'}</span>
                                      <select
                                          value={sourceEPSG}
                                  ...
                                  <div className="bg-black/30 p-3.5 rounded-xl border border-white/5 flex items-center justify-between">
                                      <span className="text-xs font-semibold">{lang === 'ar' ? 'سماكة خطوط KML' : 'KML Line Width'}</span>
                                      <div className="flex items-center gap-2">
                                          <input
                                              type="range"
                                              min={1}
                          ...
                          <div className="pt-2 border-t border-white/10 flex items-center justify-between">
                              <span className="text-xs text-white/50">{lang === 'ar' ? 'استعادة التفضيلات الأصلية' : 'Reset preferences to default'}</span>
                              <button
                                  onClick={() => {
                                      localStorage.clear();
                              ...
                              >
                                  ...
                                  <span>{lang === 'ar' ? 'إعادة ضبط التفضيلات' : 'Reset All'}</span>
                              </button>
                          </div>
                      </div>
                      ...
                      <div className="p-4 border-t border-white/10 bg-black/20 flex items-center justify-end shrink-0">
                          ...
                          >
                              {lang === 'ar' ? 'تم وحفظ الإعدادات' : 'Save & Close'}
                          </button>
                      </div>
                  </div>
              </div>
               ...
               <div className="p-4 border-t border-white/10 bg-black/20 flex items-center justify-end gap-3 shrink-0">
                 {checkResultModal.issueItems && checkResultModal.issueItems.length > 0 && (
                   ...
                   >
                     ...
                     <span>{lang === 'ar' ? 'عرض العناصر على الخريطة' : 'Show Issues on Map'}</span>
                   </button>
                 )}
                 <button
                   onClick={() => setCheckResultModal(null)}
             ...
             >
               <div className="p-6 border-b border-white/10 flex items-center justify-between bg-black/20 shrink-0">
                 <div className="flex items-center gap-3">
                   ...
                   <div>
                     <h2 className="text-lg font-black text-white">
                       {lang === 'ar' ? 'نتائج فحص التداخل والتطابق المكاني' : 'Spatial Overlap & Duplicate Analysis'}
                     </h2>
                     <p className="text-xs text-accent font-bold mt-0.5">
                       {lang === 'ar' ? `وُجدت ${overlapResults.length} حالة تطابق أو تقاطع` : `Found ${overlapResults.length} overlaps / duplicates`}
                     </p>
                   </div>
                 </div>
                 <button
               ...
               <div className="p-6 overflow-y-auto space-y-4 flex-1 custom-scrollbar text-white">
                 <div className="bg-black/30 p-4 rounded-xl border border-white/5 text-xs text-slate-300 leading-relaxed">
                   {lang === 'ar'
                     ? 'يمكنك معالجة الخطوط المتطابقة (خط فوق خط) بحذف المكرر أو تمييزها باللون الأسود لتسهيل الفحص البصري.'
                 ...
                 <div className="space-y-2 max-h-60 overflow-y-auto custom-scrollbar">
                   ...
                   {overlapResults.length > 50 && (
                     <div className="text-center text-xs text-white/40 pt-2">
                       {lang === 'ar' ? `... والمزيد (${overlapResults.length - 50} عنصر إضافي)` : `... and ${overlapResults.length - 50} more`}
                     </div>
                   )}
                 </div>
               </div>
               ...
               <div className="p-4 border-t border-white/10 bg-black/20 flex flex-wrap items-center justify-end gap-2 shrink-0">
                 ...
                 >
                   <Trash2 className="w-3.5 h-3.5" />
                   <span>{lang === 'ar' ? 'حذف الخطوط المكررة' : 'Remove Duplicates'}</span>
                 </button>
                 <button
                   onClick={() => setShowOverlapModal(false)}
